...
def plot_uncertainty_hist(y_true, p_wrong, title_prefix, plots_dir=None, bins=30):
    ...
    plt.figure()
    plt.hist(p_correct, bins=bins, alpha=0.6, color="blue", edgecolor="black",
             label=f"Correct (y=0), n={len(p_correct)}")
    plt.hist(p_wrong_vals, bins=bins, alpha=0.6, color="red", edgecolor="black",
...
def main(args):
    ...
    # VALIDATION
    X_va, y_va, pos_va, _, pred_va, rer_va = build_features_and_data(args.val_z_path, args.val_matches_dir)
    ...
    print("\n=== VALIDATION (choose p0* on VAL) ===")
    auprc_va = plot_pr_curve(y_va, p_wrong_va, f"{args.name}_VAL", plots_dir=args.plots_dir)
    print(f"[{args.name}] VAL AUPRC = {auprc_va:.3f}")
    print("Random baseline (positive rate) =", float(np.mean(y_va)))
    ...
    # TEST (fixed p0*)
    X_te, y_te, pos_te, _, pred_te, rer_te = build_features_and_data(args.test_z_path, args.test_matches_dir)
    ...
    print("\n=== TEST (use p0* from VAL) ===")
    auprc_te = plot_pr_curve(y_te, p_wrong_te, f"{args.name}_TEST", plots_dir=args.plots_dir)
    print(f"[{args.name}] TEST AUPRC = {auprc_te:.3f}")
    print("Random baseline (positive rate) =", float(np.mean(y_te)))
...
def build_arg_parser():
    p = argparse.ArgumentParser("Logistic adaptive reranking (INLIERS ONLY) with Recall@K + AUPRC")
    ...
    p.add_argument("--t_rerank", type=float, required=True)
    p.add_argument("--t_retrieval", type=float, default=0.0)
